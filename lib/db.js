...
export async function saveMateri(materi, onProgress) {
  if (USE_FIREBASE) {
    ...
    // Upload file jika raw_file tersedia
    if (materi.raw_file) {
      // Simulasi progress bar hingga 90% saat upload berjalan
      let progress = 0;
      ...
      // --- Coba upload ke Cloudinary (timeout 20 detik) ---
      try {
        const formData = new FormData();
        formData.append("file", materi.raw_file);
        formData.append("upload_preset", "sikad_preset");

        // Timeout 20 detik agar tidak stuck selamanya
        ...
        if (!response.ok) {
          console.error("Cloudinary error detail:", JSON.stringify(data));
        ...
        console.warn("Cloudinary gagal, mencoba Firebase Storage sebagai fallback:", cloudinaryError.message);
      }

      // --- Fallback 1: Firebase Storage ---
      if (!uploadSuccess) {
        try {
          const fileRef = ref(storage, `materi/${materi.id}/${materi.raw_file.name}`);
          await uploadBytes(fileRef, materi.raw_file);
          ...
          materi.storage_path = `materi/${materi.id}/${materi.raw_file.name}`;
          uploadSuccess = true;

        } catch (firebaseError) {
          console.warn("Firebase Storage gagal/non-aktif, mencoba fallback Base64:", firebaseError.message);
        }
      }

      // --- Fallback 2: Base64 Data URL (jika Cloudinary & Firebase Storage tidak aktif/perlu upgrade) ---
      if (!uploadSuccess) {
        try {
          ...
          clearInterval(progressInterval);
          console.error("Upload gagal seluruhnya:", base64Error);
          throw new Error(`Upload gagal: ${base64Error.message}`);
        }
      }

      // Hapus base64 agar tidak duplikat
      delete materi.file_data;
    }

    // Always remove raw_file to prevent Firestore serialization errors
    ...
    // Always remove raw_file to prevent serialization errors
    delete materi.raw_file;
  ...
  if (USE_FIREBASE) {
    ...
    if (snap.exists()) {
      ...
      if (data.storage_path) {
        try {
          if (data.storage_path.includes("materi_perkuliahan/")) {
            // It's a legacy Firebase Storage file
            const fileRef = ref(storage, data.storage_path);
            await deleteObject(fileRef);
          } else {
            // It's a Cloudinary file. Cloudinary unsigned delete is not supported via REST without signature.
            console.log("File is on Cloudinary (public_id: " + data.storage_path + "). Skipping physical file deletion.");
          }
        } catch (e) {
