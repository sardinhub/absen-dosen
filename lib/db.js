...
export async function saveMateri(materi, onProgress) {
  if (USE_FIREBASE) {
    ...
    if (materi.raw_file) {
      ...
      if (!uploadSuccess) {
        try {
          const { ref: storageRef, uploadBytes: fbUploadBytes, getDownloadURL: fbGetDownloadURL } = await import("firebase/storage");
          const { storage: fbStorage } = await import("./firebase");

          const fileRef = storageRef(fbStorage, `materi/${materi.id}/${materi.raw_file.name}`);
          await fbUploadBytes(fileRef, materi.raw_file);
          ...
          clearInterval(progressInterval);
          console.error("Firebase Storage juga gagal:", firebaseError);
          throw new Error(`Upload gagal: ${firebaseError.message}`);
        }
      }

      // Hapus base64 agar tidak tersimpan ke Firestore
