...
const TABLES = {
  ...
  CASIS: 'tabel_casis',
  EXAMS: 'tabel_ujian',
  EXAM_RESULTS: 'tabel_hasil_ujian'
};
...
export async function getUserByEmail(email) {
  if (USE_FIREBASE) {
    ...
    // Fallback: Check if user is a CASIS (Calon Siswa) in tabel_casis
    const qCasisUser = query(
      collection(db, TABLES.CASIS),
      where('username', '==', emailOriginal),
      limit(1)
    );
    let snapCasis = await getDocs(qCasisUser);

    if (snapCasis.empty) {
      const qCasisNiss = query(
        collection(db, TABLES.CASIS),
        where('niss', '==', emailOriginal.toUpperCase()),
        limit(1)
      );
      snapCasis = await getDocs(qCasisNiss);
    }

    if (snapCasis.empty) {
      const qCasisEmail = query(
        collection(db, TABLES.CASIS),
        where('email', '==', emailLower),
        limit(1)
      );
      snapCasis = await getDocs(qCasisEmail);
    }

    if (!snapCasis.empty) {
      const d = snapCasis.docs[0];
      const casisData = d.data();
      return {
        id: d.id,
        ...casisData,
        email: casisData.email || casisData.username,
        role: "casis"
      };
    }
    ...
    // Check local CASIS db
    const queryTerm = email.trim();
    const matchedCasis = (localDb.casis || []).find(c => 
      (c.username || '').toLowerCase() === queryTerm.toLowerCase() ||
      (c.niss || '').toUpperCase() === queryTerm.toUpperCase() ||
      (c.email || '').toLowerCase() === queryTerm.toLowerCase()
    );
    if (matchedCasis) {
      return {
        ...matchedCasis,
...
// --- CASIS CRUD ---
export async function getCasis() {
  if (USE_FIREBASE) {
    const querySnapshot = await getDocs(collection(db, TABLES.CASIS));
    const list = [];
    querySnapshot.forEach((doc) => {
    ...
    return getLocalDB().casis || [];
  }
}

export async function saveCasis(casisData) {
  if (!casisData.id) {
    casisData.id = `casis_${Date.now()}`;
  }

  // Auto-generate NISS if not provided
  if (!casisData.niss) {
    const allCasis = await getCasis();
    const currentYear = new Date().getFullYear();
    const count = allCasis.length + 1;
    casisData.niss = `NISS-${currentYear}-${String(count).padStart(3, '0')}`;
  }

  if (!casisData.status_pembayaran) {
    casisData.status_pembayaran = "LUNAS";
  }
  if (!casisData.status_konversi) {
    casisData.status_konversi = "CASIS Active";
  }
  if (!casisData.created_at) {
    casisData.created_at = new Date().toISOString();
  }

  if (USE_FIREBASE) {
    await setDoc(doc(db, TABLES.CASIS, casisData.id), casisData, { merge: true });
    return casisData;
  } else {
    const localDb = getLocalDB();
    if (!localDb.casis) localDb.casis = [];
    const idx = localDb.casis.findIndex(c => c.id === casisData.id);
    if (idx !== -1) {
      localDb.casis[idx] = { ...localDb.casis[idx], ...casisData };
    } else {
      localDb.casis.push(casisData);
    }
    saveLocalDB(localDb);
    return casisData;
  }
...
export async function deleteCasis(id) {
  if (USE_FIREBASE) {
    await deleteDoc(doc(db, TABLES.CASIS, id));
  } else {
    const localDb = getLocalDB();
    if (localDb.casis) {
      localDb.casis = localDb.casis.filter(c => c.id !== id);
      saveLocalDB(localDb);
    }
  }
}

export async function convertCasisToNim(casisId, nim, kelas = "Aero-A", angkatan = "2026") {
  const allCasis = await getCasis();
  const casis = allCasis.find(c => c.id === casisId);
  if (!casis) throw new Error("Data CASIS tidak ditemukan");

  // Create student in tabel_siswa
  const studentData = {
    id: `siswa_${Date.now()}`,
    nim: nim,
    nama_lengkap: casis.nama_lengkap,
    kelas: kelas,
    angkatan: angkatan,
    asal_sekolah: casis.asal_sekolah,
    jurusan: casis.jurusan,
    niss_asal: casis.niss,
    created_at: new Date().toISOString()
  };

  if (USE_FIREBASE) {
    await setDoc(doc(db, TABLES.STUDENTS, studentData.id), studentData);
    // Update casis status
    await setDoc(doc(db, TABLES.CASIS, casisId), {
      status_konversi: "Terkonversi ke NIM",
      nim: nim,
      converted_at: new Date().toISOString()
    }, { merge: true });
  } else {
    const localDb = getLocalDB();
    if (!localDb.siswa) localDb.siswa = [];
    localDb.siswa.push(studentData);

    if (localDb.casis) {
      const idx = localDb.casis.findIndex(c => c.id === casisId);
      if (idx !== -1) {
        localDb.casis[idx].status_konversi = "Terkonversi ke NIM";
        localDb.casis[idx].nim = nim;
        localDb.casis[idx].converted_at = new Date().toISOString();
      }
    }
    saveLocalDB(localDb);
  }
...
// --- UJIAN (EXAMS) CRUD & SCORING ---
export async function getExams(dosenId = null, kelas = null) {
  if (USE_FIREBASE) {
    let q = collection(db, TABLES.EXAMS);
    if (dosenId) {
      q = query(q, where('dosen_id', '==', dosenId));
    }
    const snap = await getDocs(q);
    const exams = [];
    snap.forEach((doc) => {
    ...
    if (kelas) {
      return exams.filter(e => !e.kelas || e.kelas === 'Semua' || e.kelas.toLowerCase() === kelas.toLowerCase());
    }
    return exams;
  } else {
    const localDb = getLocalDB();
    let exams = localDb.ujian || [];
    if (dosenId) {
      exams = exams.filter(e => e.dosen_id === dosenId);
    }
    if (kelas) {
      exams = exams.filter(e => !e.kelas || e.kelas === 'Semua' || e.kelas.toLowerCase() === kelas.toLowerCase());
    }
    return exams;
  }
}
...
export async function getExamById(id) {
  if (USE_FIREBASE) {
    const docRef = doc(db, TABLES.EXAMS, id);
    ...
    const localDb = getLocalDB();
    return (localDb.ujian || []).find(e => e.id === id) || null;
  }
}

export async function saveExam(examData) {
  const id = examData.id || `exam_${Date.now()}`;
  const record = { ...examData, id, updated_at: new Date().toISOString() };
  if (!record.created_at) record.created_at = new Date().toISOString();

  if (USE_FIREBASE) {
    await setDoc(doc(db, TABLES.EXAMS, id), record, { merge: true });
  } else {
    const localDb = getLocalDB();
    if (!localDb.ujian) localDb.ujian = [];
    const idx = localDb.ujian.findIndex(e => e.id === id);
    if (idx !== -1) {
      localDb.ujian[idx] = record;
    } else {
      localDb.ujian.push(record);
    }
    saveLocalDB(localDb);
  }
  return record;
...
export async function deleteExam(id) {
  if (USE_FIREBASE) {
    await deleteDoc(doc(db, TABLES.EXAMS, id));
  } else {
    const localDb = getLocalDB();
    if (localDb.ujian) {
      localDb.ujian = localDb.ujian.filter(e => e.id !== id);
      saveLocalDB(localDb);
    }
  }
}

export async function getExamResults(ujianId = null, siswaId = null) {
  if (USE_FIREBASE) {
    let q = collection(db, TABLES.EXAM_RESULTS);
    if (ujianId) {
      q = query(q, where('ujian_id', '==', ujianId));
    }
    const snap = await getDocs(q);
    let results = [];
    snap.forEach((doc) => {
    ...
    if (siswaId) {
      results = results.filter(r => r.siswa_id === siswaId);
    }
    return results;
  } else {
    const localDb = getLocalDB();
    let results = localDb.hasil_ujian || [];
    if (ujianId) {
      results = results.filter(r => r.ujian_id === ujianId);
    }
    if (siswaId) {
      results = results.filter(r => r.siswa_id === siswaId);
    }
...
export async function saveExamResult(resultData) {
  const id = resultData.id || `res_${Date.now()}`;
  const record = { ...resultData, id, submitted_at: new Date().toISOString() };

  if (USE_FIREBASE) {
    await setDoc(doc(db, TABLES.EXAM_RESULTS, id), record, { merge: true });
  } else {
    const localDb = getLocalDB();
    if (!localDb.hasil_ujian) localDb.hasil_ujian = [];
    const idx = localDb.hasil_ujian.findIndex(r => r.id === id);
    if (idx !== -1) {
      localDb.hasil_ujian[idx] = record;
    } else {
      localDb.hasil_ujian.push(record);
    }
    saveLocalDB(localDb);
  }
  return record;
...
/**
 * Auto-grade exam answers:
 * - Questions array contains PG (70%) and Isian (30%)
 * - PG scored by exact option key (e.g. 'A')
 * - Isian scored by key string / key list normalization (lowercase, trim, punctuation removal)
 * Returns object containing score breakdowns, item details, and final normalized grade (0-100).
 */
export function autoGradeExam(questions = [], answers = {}) {
  let totalScore = 0;
  let maxTotalScore = 0;
  let pgObtained = 0;
  let pgMax = 0;
  ...
  const itemDetails = questions.map((q, idx) => {
    const weight = Number(q.bobot) || 1;
    maxTotalScore += weight;
    const studentAns = answers[q.id || idx + 1] !== undefined ? String(answers[q.id || idx + 1]).trim() : '';
    let isCorrect = false;

    if (q.tipe === 'pg') {
      pgMax += weight;
      const key = String(q.kunci || '').trim().toUpperCase();
      isCorrect = studentAns.toUpperCase() === key;
      if (isCorrect) {
        pgObtained += weight;
        totalScore += weight;
      }
      ...
      // Isian singkat
      isianMax += weight;
      const acceptableKeys = Array.isArray(q.kunci_jawaban)
        ? q.kunci_jawaban
        : [q.kunci_jawaban || ''];
      ...
      isCorrect = acceptableKeys.some((k) => {
        const normKey = normalize(k);
        return normKey.length > 0 && (normalizedStudent === normKey || normalizedStudent.includes(normKey));
      });

      if (isCorrect) {
        isianObtained += weight;
        totalScore += weight;
      }
    }
    ...
    return {
      ...
      jawaban_siswa: studentAns,
      kunci_jawaban: q.tipe === 'pg' ? q.kunci : q.kunci_jawaban,
      is_correct: isCorrect,
      weight: weight,
      score_obtained: isCorrect ? weight : 0
    };
  });

  const finalGrade = maxTotalScore > 0 ? Math.round((totalScore / maxTotalScore) * 100) : 0;
  const pgGrade = pgMax > 0 ? Math.round((pgObtained / pgMax) * 100) : 0;
  const isianGrade = isianMax > 0 ? Math.round((isianObtained / isianMax) * 100) : 0;
