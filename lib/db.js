...
export async function saveMateri(materi, onProgress) {
  if (USE_FIREBASE) {
    ...
    // Upload to Firebase Storage if file_data is provided (base64 string) or raw_file
    if (materi.file_data || materi.raw_file) {
      ...
      let uploadTarget = materi.raw_file;
      if (!uploadTarget) {
        // Fallback for older code or local mock passing only base64
        const response = await fetch(materi.file_data);
        uploadTarget = await response.blob();
      }
