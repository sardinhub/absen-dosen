...
export async function exportSyllabusToPDF(course, lang = 'id') {
  ...
  doc.setFontSize(12);
  doc.text(lang === 'id' ? 'SILABUS PEMBELAJARAN' : 'COURSE SYLLABUS', margin, y);
  y += 10;
  ...
  y += 6;
  doc.text(`SKS: ${course.sks || '-'}`, margin, y);
  y += 6;
  ...
  if (existingSyllabus.length === 0) {
    ...
    for (let i = 0; i < existingSyllabus.length; i++) {
      ...
      doc.text(item.pertemuan ? item.pertemuan.toString() : '-', currentX + 2, y + 4);
      currentX += colWidths[0];
      ...
      const textHeight = Math.max(materiLines.length, subMateriLines.length) * 4;
      const rowHeight = Math.max(8, textHeight + 2);
