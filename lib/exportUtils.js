...
export async function exportGradesToPDF(records, lecturers, lang = 'id', action = 'save') {
  ...
  const getGradeCriteria = (score) => {
    if (score >= 95 && score <= 100) return { grade: "A+", predicate: "Terrific" };
    if (score >= 86 && score <= 94) return { grade: "A", predicate: "Excellent" };
    if (score >= 81 && score <= 85) return { grade: "A-", predicate: "Excellent" };
    if (score >= 76 && score <= 80) return { grade: "B+", predicate: "Very Good" };
    if (score >= 71 && score <= 75) return { grade: "B", predicate: "Very Good" };
    if (score >= 66 && score <= 70) return { grade: "B-", predicate: "Good" };
    if (score >= 61 && score <= 65) return { grade: "C+", predicate: "Average" };
    if (score >= 51 && score <= 60) return { grade: "C", predicate: "Average" };
    if (score >= 0 && score <= 50) return { grade: "D", predicate: "Failure" };
  ...
  for (let r = 0; r < records.length; r++) {
    ...
    // Table Headers
    const colWidths = [10, 30, 60, 25, 25, 36];
    const headers = lang === 'id' 
      ? ['NO', 'NIM', 'NAMA LENGKAP', 'SKOR DOSEN', 'GRADE', 'KRITERIA']
      : ['NO', 'NIM', 'FULL NAME', 'SCORE', 'GRADE', 'CRITERIA'];
