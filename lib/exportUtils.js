...
export async function exportGradesToPDF(records, lecturers, lang = 'id', action = 'save') {
  const { jsPDF } = await import('jspdf');
  const doc = new jsPDF('p', 'mm', 'a4');
  const pageHeight = doc.internal.pageSize.height;
  const pageWidth = doc.internal.pageSize.width;
  ...
  const addBackground = async () => {
    try {
      ...
      console.log("Kop surat tidak ditemukan.");
    }
  };
  ...
  doc.setFont('Helvetica', 'bold');
  doc.setFontSize(14);
  doc.text(lang === 'id' ? 'REKAPITULASI PENILAIAN MAHASISWA' : 'STUDENT GRADES RECAPITULATION', pageWidth / 2, y, { align: 'center' });
  y += 10;

  // Grade Criteria logic matching the page
  const getGradeCriteria = (score) => {
    if (score >= 95 && score <= 100) return { bobot: 4.0, grade: "A+", predicate: "Terrific" };
    if (score >= 90 && score <= 94) return { bobot: 3.8, grade: "A", predicate: "Excellent" };
    if (score >= 85 && score <= 89) return { bobot: 3.6, grade: "A-", predicate: "Excellent" };
    if (score >= 80 && score <= 84) return { bobot: 3.4, grade: "B+", predicate: "Very Good" };
    if (score >= 75 && score <= 79) return { bobot: 3.2, grade: "B", predicate: "Very Good" };
    if (score >= 70 && score <= 74) return { bobot: 3.0, grade: "B-", predicate: "Good" };
    if (score >= 65 && score <= 69) return { bobot: 2.75, grade: "C+", predicate: "Average" };
    if (score >= 60 && score <= 64) return { bobot: 2.5, grade: "C", predicate: "Average" };
    if (score >= 50 && score <= 59) return { bobot: 2.0, grade: "D", predicate: "Failure" };
    if (score >= 30 && score <= 49) return { bobot: 1.0, grade: "E", predicate: "Failure" };
    if (score >= 0 && score <= 29) return { bobot: 0.0, grade: "K", predicate: "Empty" };
    return { bobot: 0.0, grade: "-", predicate: "-" };
  };

  if (records.length === 0) {
    doc.setFont('Helvetica', 'normal');
    doc.setFontSize(10);
    doc.text(lang === 'id' ? 'Tidak ada data penilaian.' : 'No grade data available.', margin, y);
    if (action === 'preview') return doc.output('bloburl');
  ...
  for (let r = 0; r < records.length; r++) {
    ...
    // Check page height for class header
    if (y + 25 > pageHeight - 35) {
      doc.addPage();
      await addBackground();
      y = 45;
    }
    ...
    doc.setFontSize(10);
    doc.setTextColor(0, 0, 0);
    doc.text(`${lang === 'id' ? 'Mata Kuliah' : 'Course'}: ${rec.mk_nama || '-'}`, margin, y);
    y += 5;
    doc.setFont('Helvetica', 'normal');
    doc.setFontSize(9);
    doc.text(`Dosen: ${lecturers[rec.dosen_id] || rec.dosen_id}`, margin, y);
    doc.text(`Kelas: ${rec.kelas || '-'}`, margin + 80, y);
    y += 5;

    // Table Headers
    const colWidths = [10, 25, 55, 20, 15, 20, 36];
    const headers = lang === 'id' 
      ? ['NO', 'NIM', 'NAMA LENGKAP', 'SKOR', 'BOBOT', 'GRADE', 'KRITERIA']
      : ['NO', 'NIM', 'FULL NAME', 'SCORE', 'WEIGHT', 'GRADE', 'CRITERIA'];
    ...
    let studentsOnPage = 0;

    if (siswaList.length === 0) {
      y += 5;
      doc.text(lang === 'id' ? 'Belum ada data siswa.' : 'No students data.', margin + 2, y);
      y += 5;
    } else {
      for (let s = 0; s < siswaList.length; s++) {
        const siswa = siswaList[s];
        
        if (y + 10 > pageHeight - 35 || studentsOnPage >= 20) {
          doc.addPage();
          await addBackground();
          y = 45;
          studentsOnPage = 0;
        ...
        const criteria = getGradeCriteria(siswa.score);
        currentX = margin;
        
        y += 5;
        doc.text((s + 1).toString(), currentX + 2, y);
        currentX += colWidths[0];
        doc.text(siswa.nim || '-', currentX + 2, y);
        currentX += colWidths[1];
        doc.text(siswa.nama || '-', currentX + 2, y);
        currentX += colWidths[2];
        doc.text((siswa.score || 0).toString(), currentX + 2, y);
        currentX += colWidths[3];
        doc.text(criteria.bobot.toString(), currentX + 2, y);
        currentX += colWidths[4];
        doc.setFont('Helvetica', 'bold');
        doc.text(criteria.grade, currentX + 2, y);
        doc.setFont('Helvetica', 'normal');
        currentX += colWidths[5];
        doc.text(criteria.predicate, currentX + 2, y);
    ...
    y += 10; // Spacing before next class
  }
  ...
  if (action === 'preview') {
    ...
    doc.save('rekap_penilaian_mahasiswa.pdf');
    return null;
  }
}
